...
function byCodePoint([a]: [string, unknown], [b]: [string, unknown]) {
...
export async function writeLanguage(code: string, contents: LanguageFile) {
  ...
  // Keys sorted so an edit produces a one-line diff rather than a reordered file — by code point,
  // matching Python's `sorted()`, so a write from here and a write from apply-translations.py
  // agree. `localeCompare` would order them differently and every edit would reshuffle the file.
  const ordered: LanguageFile = {
    ...contents,
    translations: Object.fromEntries(Object.entries(contents.translations).sort(byCodePoint)),
    plurals: Object.fromEntries(Object.entries(contents.plurals).sort(byCodePoint)),
  }
...
export async function writeVocabulary(vocabulary: Vocabulary) {
  ...
  // And the generated copy, in the exact shape scripts/build-vocabulary.py emits. Writing only the
  // Markdown would leave the JSON stale until someone remembered to run the generator, and the
  // JSON is what this site and the app's tests actually read. `VocabularyTests` compares the two,
  // so a drift between this writer and the Python one is caught rather than silently shipped.
  const generated = {
    $comment: "Generated from VOCABULARY.md by scripts/build-vocabulary.py. Do not edit.",
  ...
  await writeFile(
    path.join(appRepositoryPath, "vocabulary.json"),
