...
export const metadata: Metadata = { title: "Kasse · chordlink", robots: { index: false, follow: false } }

export default async function Page() {
  const availability = await fetchChordlinkAvailability()
  if (!mayOpenChordlinkCheckout(availability)) redirect("/de/chordlink?checkout=unavailable")
  ...
  const session = await createChordlinkCheckoutSession("de", baseUrl)
  if (!session) redirect("/de/chordlink?checkout=unavailable")

  return <ChordlinkCheckoutPage clientSecret={session.clientSecret} language="de" publishableKey={session.publishableKey} />
}
